...
> [!NOTE]
> Qt Visual Studio Tools helps Visual Studio integration, but CMake package discovery (`find_package(Qt6 ...)`) still depends on an installed Qt **MSVC** kit.

### Qt troubleshooting (if `find_package(Qt6 ...)` fails)

If you see an error like `Could not find Qt6Config.cmake`, check the installed kits:

```powershell
Get-ChildItem C:\Qt -Recurse -Filter Qt6Config.cmake
```

For this build (MSVC/clang-cl), Qt must be in a path like:

```text
C:\Qt\<QtVersion>\msvc2022_64\lib\cmake\Qt6\Qt6Config.cmake
```

If you only have `mingw_64`, install `msvc2022_64` using Qt Maintenance Tool.

You can also set Qt explicitly when configuring:

```powershell
cmake -S . -B build/x64-Clang-Release-Qt -G Ninja `
   -DCMAKE_C_COMPILER=clang-cl -DCMAKE_CXX_COMPILER=clang-cl `
   -DENABLE_QT_GUI=ON `
   -DCMAKE_PREFIX_PATH="C:/Qt/<QtVersion>/msvc2022_64"
```
...
cmake -S . -B build/x64-Clang-Release-Qt -G Ninja `
   -DCMAKE_C_COMPILER=clang-cl -DCMAKE_CXX_COMPILER=clang-cl `
   -DENABLE_QT_GUI=ON `
   -DQt6_DIR="C:/Qt/<QtVersion>/msvc2022_64/lib/cmake/Qt6"
```
